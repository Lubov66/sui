...
impl AncestorStateManager {
    ...
    // Chose 750 rounds worth of updates with at ~15 rounds per second conservatively
    // is ~50 second of waiting which is similar to the round prober updates.
    // Additionally, this number will be multiplied by the number of authorities in the
    // committee to get the total number of blocks that need to be accepted before we unlock.
    #[cfg(not(test))]
    const STATE_LOCK_NEW_BLOCK_UPDATES: u32 = 750;
    #[cfg(test)]
    const STATE_LOCK_NEW_BLOCK_UPDATES: u32 = 1;
    ...
    pub(crate) fn update_all_ancestors_state(
        ...
        for (authority_id, score) in propagation_scores_by_authority {
            let (_low, authority_high_quorum_round) = accepted_quorum_rounds[authority_id];
    ...
    fn update_state(
        ...
        round_prober_update_count: u32,
        // TODO(arun): Use a combination of prober updates and new block updates?
        new_block_update_count: u32,
    ) {
            ...
            AncestorState::Include => {
                if propagation_score <= low_score_threshold {
                    ...
                    let new_block_update_expiry_count = new_block_update_count
                        + Self::STATE_LOCK_NEW_BLOCK_UPDATES * self.context.committee.size() as u32;
                    ...
                    info!(
                        "Authority {authority_id} moved to EXCLUDE state with score {propagation_score} <= threshold of {low_score_threshold} and locked for {:?} quorum round updates or {:?} new block updates",
...
mod test {
    ...
    async fn test_update_all_ancestor_state_using_accepted_rounds() {
        ...
        ancestor_state_manager.update_all_ancestors_state(accepted_quorum_rounds.clone(), 1, 0);
        ...
        ancestor_state_manager.update_all_ancestors_state(accepted_quorum_rounds.clone(), 3, 0);
